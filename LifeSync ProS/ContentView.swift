...
struct ContentView: View {
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            ...
            WeightTrackerView(userDataManager: userDataManager)
                .tabItem {
            ...
            GermanStudyView(userDataManager: userDataManager)
                .tabItem {
        ...
        .accentColor(.blue)
        .onAppear {
            // Check if it's a new day and reset daily progress if needed
    ...
    private func checkAndResetDailyProgress() {
        ...
        if let lastResetDateData = defaults.object(forKey: "lastResetDate") as? Date {
            // If it's a new day, reset daily progress
            if !calendar.isDate(lastResetDateData, inSameDayAs: today) {
                userDataManager.resetDailyProgress()
                defaults.set(today, forKey: "lastResetDate")
            }
        } else {
            // First launch, set the reset date
            defaults.set(today, forKey: "lastResetDate")
        }
    }
}
...
class UserDataManager: ObservableObject {
    ...
    @Published var germanStudySessions: [StudySession] = []
    @Published var b2ExamDate: Date
    ...
    init() {
        // Set default exam date (60 days from now)
        self.b2ExamDate = Calendar.current.date(byAdding: .day, value: 60, to: Date()) ?? Date()
    ...
    func addStudySession(duration: Int, type: StudyType, date: Date = Date()) {
        let session = StudySession(date: date, duration: duration, type: type)
        germanStudySessions.append(session)
        updateStudyStreak()
        saveData()
    }
    ...
    func saveGermanTodoItems(_ items: [StudyTask]) {
        // Implementation to save German todo items
        // This would need to be added to the data persistence mechanism
        saveData()
    }
    ...
    private func updateStudyStreak() {
        ...
        let calendar = Calendar.current
        let sortedSessions = germanStudySessions.sorted { $0.date > $1.date }
    ...
    private func loadData() {
        ...
        if let studyData = defaults.data(forKey: germanStudyKey),
           let decodedStudyProgress = try? JSONDecoder().decode([StudySession].self, from: studyData) {
            germanStudySessions = decodedStudyProgress
            updateStudyStreak()
        }
        
        // Load exam date
        if let examDateTimeInterval = defaults.object(forKey: examDateKey) as? TimeInterval {
            b2ExamDate = Date(timeIntervalSince1970: examDateTimeInterval)
        }
    ...
    func saveData() {
        ...
        // Save study data
        if let encodedStudyProgress = try? JSONEncoder().encode(germanStudySessions) {
            defaults.set(encodedStudyProgress, forKey: germanStudyKey)
        }
        
        // Save exam date
        defaults.set(b2ExamDate.timeIntervalSince1970, forKey: examDateKey)
    }
